...
- [Tests for `microbetag` basic modules](#tests-for-microbetag-basic-modules)
  - [Infer co-occurrence network with FlashWeave (`test_flashweave`)](#infer-co-occurrence-network-with-flashweave-test_flashweave)
  - [Literature based node annotation with FAPROTAX (`test_faprotax`)](#literature-based-node-annotation-with-faprotax-test_faprotax)
  - [Genome based node annotations using `phenotrex` predictions (`test_phenotrex`)](#genome-based-node-annotations-using-phenotrex-predictions-test_phenotrex)
  - [Gene prediction using Prodigal (`test_prodigal`)](#gene-prediction-using-prodigal-test_prodigal)
  - [KEGG ORTHOLOGY annotation (`test_kegg_annotation`)](#kegg-orthology-annotation-test_kegg_annotation)
  - [Pathway complementarity extraction (`test_path_compl`)](#pathway-complementarity-extraction-test_path_compl)
  - [Genome-scale metabolic reconstruction with ModelSEED (`test_modelseed`)](#genome-scale-metabolic-reconstruction-with-modelseed-test_modelseed)
  - [Genome-scale metabolic reconstruction with CarveMe (`test_carve`)](#genome-scale-metabolic-reconstruction-with-carveme-test_carve)
  - [Seed complementarity extraction (`test_seed_compl`)](#seed-complementarity-extraction-test_seed_compl)
  - [Network clustering using `manta` (`test_manta`)](#network-clustering-using-manta-test_manta)
  - [Complete `microbetag` run (`test_microbetag`)](#complete-microbetag-run-test_microbetag)
...
## Infer co-occurrence network with FlashWeave ([`test_flashweave`](./test_flashweave//))
...
## Literature based node annotation with FAPROTAX ([`test_faprotax`](./test_faprotax/))

Builds the merged `functional_otu_table.tsv` and the `sub_tables` folder with the `.tsv` files per trait found, 
as FAPROTAX does.


## Genome based node annotations using `phenotrex` predictions ([`test_phenotrex`](./test_phenotrex))
...
## Gene prediction using Prodigal ([`test_prodigal`](./test_prodigal))
...
## KEGG ORTHOLOGY annotation ([`test_kegg_annotation`](./test_kegg_annotation))
...
## Pathway complementarity extraction ([`test_path_compl`](./test_path_compl))
...
## Genome-scale metabolic reconstruction with ModelSEED ([`test_modelseed`](./test_modelseed))
...
## Genome-scale metabolic reconstruction with CarveMe ([`test_carve`](./test_carve))
...
## Seed complementarity extraction ([`test_seed_compl`](./test_seed_compl))
...
## Network clustering using `manta` ([`test_manta`](./test_manta))
...
## Complete `microbetag` run ([`test_microbetag`](./test_microbetag/))
