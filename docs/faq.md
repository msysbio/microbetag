...
In case you already have a network, and you have loaded it to MGG as described in the [relative tutorial](advanced_use//load.md) then you may set this parameter to `network` so `microbetag` will annotate the provided network. 
...
As a rule-of-thumb, the `sensitive` module requires further computing resources and time, thus it may lead the online version of `microbetag` to time errors. In this case the user can always go for the [`microbetag_prep`](advanced_use//prep.md) steps to infer the network locally. 
...
{: .note}
We remind that it is always a good practice in terms of getting as many and as good as possible annotations to run the [`microbetag_prep`](advanced_use//prep.md) step instead of using the `Other` taxonomy.
...
In this case, you should first run the [`microbetag-prep`](advanced_use//prep.md) step and use its output as your input files. 
...
When running `microbetag` locally (see [relative tutorial](advanced_use//local)) one may have a wide range of different input files as starting points.
...
There is a great chance when you are trying to reconstruct Genome Scale Models (GEMs) using your own genomes/bins/MAGs and the `modelseedpy` library, as shown in the relative [tutorial](advanced_use//local.md), to keep getting messages like:
